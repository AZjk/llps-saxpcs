...
# elapsed for every B0147 file so the colour scale matches the main figure (0..max)
elapsed_all = {fp: (start_times[fp] - t_ref).total_seconds() for fp in b0147}
elapsed = {fp: elapsed_all[fp] for fp in xpcs_files}
norm = Normalize(vmin=0, vmax=max(elapsed_all.values()))
...
# One shared legend on the right for the elapsed-time colours.
fig.legend(handles, [h.get_label() for h in handles], loc='center left',
           bbox_to_anchor=(1.0, 0.5), fontsize=11, title='Elapsed time', frameon=False)
