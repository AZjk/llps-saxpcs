...
gpu_corr_worker () {
    ...
    do
        echo running $RAWFILE
		# run with all frames
		boost_corr -r $RAWFILE -q $QMAP  -v -i $1 -ow -o './results/cluster_results'
		# run with the first 50,000 frames
		boost_corr -r $RAWFILE -q $QMAP  -v -i $1 -ow -o './results/cluster_results_part1' -begin_frame 0 -end_frame 50000
		# run with the last 50,000 frames
		boost_corr -r $RAWFILE -q $QMAP  -v -i $1 -ow -o './results/cluster_results_part2' -begin_frame 50000 -end_frame 100000
    done
}
